...
(function ($, Drupal) {
  ...
  mappy_tag.once('mappy').each(function (index) {
    ...
    mappy[index] = {
      ...
      width: mappy_instance.get(0)
        .getAttribute("width") > 0 ? mappy_instance.get(0)
        .getAttribute("width") : Drupal.settings.mappy_yandex_width,
      // Height of the map. If not present, we get default value from settings.
      height: mappy_instance.get(0)
        .getAttribute("height") > 0 ? mappy_instance.get(0)
        .getAttribute("height") : Drupal.settings.mappy_yandex_height,
      // Scale of the map.
      zoom: mappy_instance.get(0)
        .getAttribute("zoom") > 0 ? parseInt(mappy_instance.get(0)
        .getAttribute("zoom")) : 17,
      // Zoom button (X,Y).
      zoomControl: mappy_instance.get(0)
        .getAttribute("zoomControl") ? mappy_instance.get(0)
        .getAttribute("zoomControl")
        .split(",") : false,
      // Small zoom button (X,Y).
      smallZoomControl: mappy_instance.get(0)
        .getAttribute("smallZoomControl") ? mappy_instance.get(0)
        .getAttribute("smallZoomControl")
        .split(",") : false,
      // Disable balloons.
      addressPlacemark: (mappy_instance.get(0)
        .getAttribute("addressPlacemark") == "false") ? false : true,
      // If TRUE: users can choose layers.
      mapTypeControl: mappy_instance.get(0)
        .getAttribute("mapTypeControl") == "true" ? true : false,
      // If TRUE: display button for controlling jams on road.
      mapTrafficControl: mappy_instance.get(0)
        .getAttribute("mapTrafficControl") == "true" ? true : false,
      // Content for balloons.
      balloonContent: (mappy_instance.attr("balloonContent")) ? mappy_instance.attr("balloonContent")
        .split(";") : false,
      // Route controllers.
      route: mappy_instance.get(0).getAttribute("route") ? mappy_instance.get(0)
        .getAttribute("route")
        .split(",") : false,
      // If TRUE: balloons will merge into one big, before they zoomed enough for seeing separately.
      cluster: mappy_instance.get(0)
        .getAttribute("clusters") == "true" ? true : false
    };
    ...
    // Else, we search coordinates by API.
      ...
      $.ajax({
        ...
        success: function (data) {
          mappy[index].centerCoordinates = data.response.GeoObjectCollection.featureMember[0].GeoObject.Point.pos.split(" ")
            .reverse();
          create_map();
        }
      });
    ...
    function create_map() {
      ...
      mappy_instance.addClass('mappy yandex');
      ...
      function init() {
        ...
        if (mappy[index].address.length > 0) {
          ...
          for (var i = 0; i < mappy[index].address.length; i++) {
            ...
            $.ajax({
              ...
              success: function (data) {
                geocoding_coord = data.response.GeoObjectCollection.featureMember[0].GeoObject.Point.pos.split(" ")
                  .reverse();
        ...
        if (mappy[index].zoomControl) {
          maps[index].map.controls
            .add('zoomControl', {
              left: mappy[index].zoomControl[0],
              top: mappy[index].zoomControl[1]
            });
        }
        ...
        if (mappy[index].route) {
          ...
          $("#" + mappy[index].route[0]).click(function () {
            ...
            ymaps.route([
              ...
              points.get(0)
                .properties
                .set('iconContent', Drupal.t('Your location'));
              // Icon and text of 'Where' bubble.
              points.get(1)
                .properties
                .set('iconContent', Drupal.t("We're here!"));
            }, function (error) {
